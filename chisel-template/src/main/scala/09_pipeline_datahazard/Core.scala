...
class Core extends Module {
  ...
  val if_pc_next = MuxCase(if_pc_plus4, Seq(  // execution priority :: jump > stall
  ...
  // IF/ID Register
  id_reg_pc := if_reg_pc
  id_reg_inst := MuxCase(if_inst, Seq(  // execution priority :: jump > stall
    (exe_br_flg || exe_jmp_flg) -> BUBBLE, // if JUMP or BRANCH, disable IF Stage
  ...
  // if JUMP or BRANCH, disable ID Stage
  ...
  val id_rs1_data = MuxCase(regfile(id_rs1_addr), Seq(
    ...
    ((id_rs1_addr === mem_reg_wb_addr) && (mem_reg_rf_wen === REN_S)) -> mem_wb_data, // forwarding from MEM
    ((id_rs1_addr === wb_reg_wb_addr) && (wb_reg_rf_wen === REN_S)) -> wb_reg_wb_data // forwarding from WB
  ))
  val id_rs2_data = MuxCase(regfile(id_rs2_addr), Seq(
    (id_rs2_addr === 0.U) -> 0.U(WORD_LEN.W),
    ((id_rs2_addr === mem_reg_wb_addr) && (mem_reg_rf_wen === REN_S)) -> mem_wb_data, // forwarding from MEM
    ((id_rs2_addr === wb_reg_wb_addr) && (wb_reg_rf_wen === REN_S)) -> wb_reg_wb_data // forwarding from WB
  ))
  ...
  val csr_wdata = MuxCase(0.U(WORD_LEN.W), Seq(
    ...
    (mem_reg_csr_cmd === CSR_E) -> 11.U(WORD_LEN.W) // WBdata of ECALL is 8 + prv mode(=3:machine mode)
  ))
  // CSR case
